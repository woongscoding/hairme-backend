#!/usr/bin/env python3
"""
v4 모델 학습 스크립트 - 데이터 리키지 방지 버전

**핵심 차이점:**
- 얼굴 단위로 train/val/test 분할 (샘플 단위 X)
- 같은 얼굴에서 생성된 6개 샘플이 모두 같은 split에 속함
...
def create_dataloaders_no_leakage(
    ...
    Args:
        ...
        samples_per_face: 얼굴당 샘플 개수 (기본 6)
    ...
    logger.info(f"\n📊 샘플 단위 분할:")
    logger.info(f"  - Train: {len(train_sample_indices):,}개 샘플 ({len(train_sample_indices)/total_samples*100:.1f}%)")
    logger.info(f"  - Val:   {len(val_sample_indices):,}개 샘플 ({len(val_sample_indices)/total_samples*100:.1f}%)")
    logger.info(f"  - Test:  {len(test_sample_indices):,}개 샘플 ({len(test_sample_indices)/total_samples*100:.1f}%)")
    ...
    # 검증: 중복 확인
    all_indices = set(train_sample_indices + val_sample_indices + test_sample_indices)
    assert len(all_indices) == total_samples, "샘플 누락 또는 중복!"
    assert len(set(train_sample_indices) & set(val_sample_indices)) == 0, "Train-Val 중복!"
    assert len(set(train_sample_indices) & set(test_sample_indices)) == 0, "Train-Test 중복!"
    assert len(set(val_sample_indices) & set(test_sample_indices)) == 0, "Val-Test 중복!"

    logger.info(f"✅ 데이터 리키지 검증 완료: 중복 없음!")
...
def train_model(
    ...
    """
    logger.info("=" * 60)
    logger.info("🚀 v4 모델 학습 시작 (데이터 리키지 방지)")
    logger.info("=" * 60)
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val_loss:
            ...
            model_path = output_dir / "hairstyle_recommender_v4_no_leakage.pt"
            torch.save({
                'epoch': epoch + 1,
    ...
    # 학습 기록 저장
    history_path = output_dir / "training_history_v4_no_leakage.json"
    ...
    logger.info(f"  - Test MAE: {test_mae:.2f}")
    logger.info(f"  - 모델 경로: {output_dir / 'hairstyle_recommender_v4_no_leakage.pt'}")


def main():
    parser = argparse.ArgumentParser(description="v4 모델 학습 (데이터 리키지 방지)")
    parser.add_argument(
        "--data",
        type=str,
        required=True,
