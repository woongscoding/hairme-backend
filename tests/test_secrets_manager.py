...
from config.secrets import get_secret, is_aws_environment, get_secret_or_env
...
class TestSecretsManager:
    ...
    def test_get_secret_or_env_aws_environment(
        self, mock_get_secret, mock_is_aws, monkeypatch
    ):
    ...
    @patch("config.secrets.is_aws_environment")
    @patch("config.secrets.get_secret")
    def test_get_secret_or_env_fallback_to_env(
        self, mock_get_secret, mock_is_aws, monkeypatch
    ):
    ...
    @patch("config.secrets.is_aws_environment")
    @patch("config.secrets.get_secret")
    def test_get_secret_or_env_required_missing(
        self, mock_get_secret, mock_is_aws, monkeypatch
    ):
    ...
    @patch("config.secrets.is_aws_environment")
    @patch("config.secrets.get_secret")
    def test_get_secret_or_env_optional_missing(
        self, mock_get_secret, mock_is_aws, monkeypatch
    ):
...
class TestSettingsIntegration:
    ...
    def test_settings_loads_from_secrets_manager_in_aws(
        ...
        def mock_secret_fetch(
            secret_name, env_var_name, region_name="ap-northeast-2", required=True
        ):
            secrets = {
